...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class<? extends Message>,Callback> callMyMessage;
    ...
    public MicroService(String name) {
        this.name = name;
        callMyMessage=new ConcurrentHashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        callMyMessage.put(type,callback);
     ...
     * each micro service registers and then waits for messages, for each message it response by sending a callback
     * terminates and then unregisters
     */
    @Override
    public final void run() {
        ...
        while (!terminated) {
           try {
               Message receivedMessage=MessageBusImpl.getInstance().awaitMessage(this);
               callMyMessage.get(receivedMessage.getClass()).call(receivedMessage);
           }
